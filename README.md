...
poll count, accepted rows, endpoint readiness, observed token progress, and
...
print per-request token/target progress for bounded batch streams and
`next[...]` lines with safe follow-up commands. Human `infer` and
