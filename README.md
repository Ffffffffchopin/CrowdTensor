...
metadata. `answer_scope` distinguishes terminal-visible answers from saved
reports; the Markdown `What To Do Next` and `Details` sections repeat that
saved JSON and Markdown contain no generated text. `local_output` includes safe
output `count` and `source` fields such as
