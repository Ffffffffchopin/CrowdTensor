...
- Safe two-machine remote demo acceptance pack (`scripts/remote_demo_acceptance_pack.py`, `scripts/remote_demo_acceptance_check.py`) with `remote_demo_acceptance_v1`, wait-for-result behavior, `remote_compute_evidence_v1`, `support_bundle` collection, and stable `diagnosis_codes` such as `coordinator_unreachable`, `observer_auth_failed`, and `artifact_collection_failed`.
