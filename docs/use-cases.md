...
- Run `scripts/remote_compute_evidence_pack.py` when you want a safe, shareable `remote_compute_evidence_v1` artifact with `remote_compute_observability_v1` showing a registry-backed remote-style Miner completing read-only `model_bundle_infer`.
...
- Run `scripts/remote_demo_acceptance_pack.py --create-session` after the two-machine demo is running to call `POST /admin/inference-sessions`, wait for the returned `task_id`, and collect `remote_demo_acceptance_v1`, `remote_compute_evidence_v1`, `remote_demo_observability_v1`, `support_bundle`, and `diagnosis_codes` such as `coordinator_unreachable`, `observer_auth_failed`, `session_create_failed`, and `artifact_collection_failed`.
