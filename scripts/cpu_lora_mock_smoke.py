#!/usr/bin/env python3
"""Subprocess smoke test for the CrowdTensorD cpu_lora_mock workload."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run cpu_lora_mock workload smoke test.")
    ...
    parser.add_argument("--miner-timeout", type=float, default=15.0)
...
def main() -> None:
    ...
    if state_dir is None:
        temp_dir = tempfile.TemporaryDirectory(prefix="crowdtensor_cpu_lora_")
    ...
    try:
        ...
        if completed.returncode != 0:
            raise SystemExit(f"miner failed code={completed.returncode}\n{completed.stdout}")

        state = request_json("GET", args.base_url, "/state")
        model = state.get("model", {})
        if state.get("accepted_results") != 1 or state.get("adapter_updates") != 1:
            raise SystemExit(f"unexpected lora state: {json.dumps(state, sort_keys=True)}")
        if int(model.get("global_step", -1)) != 0 or int(model.get("adapter_step", -1)) != 1:
            raise SystemExit(f"unexpected model counters: {json.dumps(model, sort_keys=True)}")

        print(json.dumps({
            "accepted_results": state["accepted_results"],
            "adapter_updates": state["adapter_updates"],
            "model_updates": state["model_updates"],
            "global_step": model["global_step"],
            "adapter_step": model["adapter_step"],
            "lora_values": model["lora_adapter"]["values"],
        }, sort_keys=True))
    finally:
