...
def validate_report(report: dict[str, Any], *, require_core_ready: bool = False) -> None:
    ...
    if stage_selective.get("ready"):
        ...
        if stage_selective.get("stage_selective_hf_runtime_ready") and truth.get("stage_selective_hf_runtime_is_not_7b_8b_completion") is not True:
            raise SystemExit("stage-selective HF runtime must not be treated as 7B/8B completion")
