...
def validate_payload(payload: dict[str, Any], *, mode: str) -> list[str]:
    ...
    if user_status.get("state") not in {"ready", "blocked", "package-blocked", "live-blocked"}:
        errors.append("user_status_state_missing")
    ...
    if review.get("schema") != "public_swarm_operator_preview_review_summary_v1":
    ...
    if not review.get("recommended_label"):
        errors.append("review_summary_recommended_label_missing")
