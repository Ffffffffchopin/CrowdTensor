#!/usr/bin/env python3
"""Validate a public-safe bounded Colab TPU reacquire retry report."""
...
def validate(report: dict[str, Any], *, require_ready: bool = False) -> list[str]:
    ...
    for index, attempt in enumerate(attempts):
        ...
        if str(attempt.get("accelerator_requested") or "") not in {"V5E1", "V6E1"}:
            errors.append(f"attempt {index} unexpected accelerator")
    ...
    ready = report.get("colab_tpu_reacquire_ready") is True
    if require_ready and not ready:
        errors.append("Colab TPU reacquire was not verified")
    if ready:
