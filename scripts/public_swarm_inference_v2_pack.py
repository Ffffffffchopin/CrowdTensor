...
def prompt_scope_summary(args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        ...
        "public_artifact_safe": True,
        "summary": (
            "This Public Swarm v2 aggregate records prompt source/count and placeholder safety only; "
            "raw prompt text and prompt file paths are excluded from public JSON, Markdown, and support bundles."
        ),
    }
...
def v2_gpu_verdict_state(cuda: dict[str, Any], provenance: dict[str, Any]) -> str:
    if cuda.get("ready") is True and provenance.get("retained_gpu_evidence_imported") is True:
        return "retained-gpu-evidence"
    if cuda.get("ready") is True:
        return "gpu-runtime-ready"
    if cuda.get("fail_closed_ready") is True and cuda.get("cuda_unavailable") is True:
        return "cuda-unavailable-fail-closed"
    if cuda.get("fail_closed_ready") is True:
        return "cuda-fail-closed"
    return "gpu-evidence-blocked"
...
def inference_verdict_summary(report: dict[str, Any]) -> dict[str, Any]:
    ...
    completed = bool(report.get("ok") is True and local.get("ready") is True)
    state = str(review.get("state") or user_status.get("state") or ("ready" if report.get("ok") else "blocked"))
    blocked = state == "blocked" or report.get("ok") is not True
    preflight_only = str(report.get("mode") or "") == MODE_PACKAGE
    generated = int(local.get("generated_token_count") or 0)
    max_new_tokens = int(local.get("max_new_tokens") or 0)
    fresh_external_verified = provenance.get("fresh_external_verified") is True
    retained_gpu_imported = provenance.get("retained_gpu_evidence_imported") is True
    gpu_state = v2_gpu_verdict_state(cuda, provenance)
    if completed:
        message = (
            "Public Swarm v2 inference evidence is ready. This report proves the redacted "
            "route/evidence path; it does not save a local answer transcript."
        )
    elif preflight_only:
        message = "Public Swarm v2 package artifacts are ready, but no local inference proof was run in this mode."
    else:
        message = "Public Swarm v2 inference evidence is blocked; follow the recommended next command."
    return {
        ...
        "blocked": blocked,
        "result_status": "ready" if completed else ("package-only" if preflight_only else "blocked"),
        "generated_token_count": generated,
        "max_new_tokens": max_new_tokens,
        "output_count": int(local.get("accepted_rows") or 0),
        ...
        "evidence_level": provenance.get("proof_level") or report.get("mode") or "unknown",
        "executed_where": provenance.get("proof_level") or report.get("mode") or "unknown",
        "gpu_state": gpu_state,
    ...
    *,
    ...
    if requires_private_credentials:
        ...
        entry["credential_note"] = (
            "Prepare the private admin/miner credentials from the runbook before running this command; "
...
def artifact_summary(output_dir: Path) -> dict[str, Any]:
    paths = {
        "inspect_first": output_dir / "public_swarm_inference_v2.md",
        "summary_json": output_dir / "public_swarm_inference_v2.json",
        "summary_markdown": output_dir / "public_swarm_inference_v2.md",
        "support_bundle": output_dir / "support_bundle.json",
        "runbook": output_dir / "PUBLIC_SWARM_INFERENCE_V2.md",
    }
...
def public_swarm_v2_command(args: argparse.Namespace, output_dir: Path, mode: str) -> list[str]:
    command = [
        "crowdtensor",
        "public-swarm-v2",
        mode,
    ...
    if mode in {MODE_LOCAL, MODE_LOCAL_MODEL_VARIANT}:
        command.extend([
            "--public-host",
            ...
            str(args.real_p2p_discovery_backend),
            "--startup-timeout",
    ...
    if mode == MODE_EVIDENCE_IMPORT:
        command.extend([
            "--usable-report",
            str(args.usable_report),
            "--preview-report",
            str(args.preview_report),
            "--real-p2p-report",
            str(args.real_p2p_report),
            "--gpu-report",
            str(args.gpu_report),
        ])
        if args.fresh_external_report:
            command.append("--fresh-external-report")
        if args.fresh_external_attempt_report:
            command.extend(["--fresh-external-attempt-report", str(args.fresh_external_attempt_report)])
    if args.hf_model_id != DEFAULT_HF_MODEL_ID:
        command.extend(["--hf-model-id", args.hf_model_id])
    if args.backend != "cpu":
        command.extend(["--backend", args.backend])
...
def recommended_next_command(
    ...
    *,
    ...
    return {
        ...
        "credential_note": (
            "Prepare the private admin/miner credentials from the runbook before rerunning; "
            "credential variable names and values are kept out of public artifacts."
        ),
        "public_artifact_safe": True,
    }
    ...
    *,
    ...
    commands = [
        command_entry(
            ...
            artifact_command(output_dir, "public_swarm_inference_v2.md"),
            reason="review_artifacts",
        ),
        command_entry(
            "open v2 runbook",
            artifact_command(output_dir, "PUBLIC_SWARM_INFERENCE_V2.md", lines="1,260p"),
            reason="follow_public_swarm_v2_path",
        ),
    ]
    if mode == MODE_PACKAGE:
        commands.append(command_entry(
            "run local v2 proof",
            public_swarm_v2_command(args, output_dir, MODE_LOCAL),
            reason="execute_local_p2p_path",
            requires_private_credentials=True,
        ))
    elif ready:
        commands.append(command_entry(
            "rerun local v2 proof",
            public_swarm_v2_command(args, output_dir, MODE_LOCAL),
            reason="refresh_local_p2p_path",
            requires_private_credentials=True,
        ))
    elif recommended.get("reason") == "refresh_external_p2p_evidence":
        commands.append(command_entry(
            "import fresh external evidence",
            public_swarm_v2_command(args, output_dir, MODE_EVIDENCE_IMPORT),
            reason="refresh_external_p2p_evidence",
        ))
    else:
        commands.append(command_entry(
            "retry local v2 proof",
            public_swarm_v2_command(args, output_dir, MODE_LOCAL),
            reason="fix_local_v2_evidence",
            requires_private_credentials=True,
        ))
...
def build_common_report(
    ...
    *,
    ...
    report = {
        ...
        "next_commands": report_commands,
        "artifact_summary": artifact_summary(output_dir),
        "operator_action": [
...
def validate_public_report(report: dict[str, Any]) -> list[str]:
    ...
    if any(str(item).startswith("CROWDTENSOR_") for item in (recommended.get("requires_env") or [])):
        errors.append("recommended_next_command_requires_env_public_secret_name")
    if not next_items:
    ...
    for item in next_items:
        ...
        if any(str(env_name).startswith("CROWDTENSOR_") for env_name in (item.get("requires_env") or [])):
            errors.append("next_command_requires_env_public_secret_name")
    ...
    if verdict.get("state") not in {"ready", "blocked"}:
        errors.append("inference_verdict_state_mismatch")
    if verdict.get("completed") is not bool(report.get("ok") is True):
