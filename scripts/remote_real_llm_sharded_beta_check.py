#!/usr/bin/env python3
"""CI-safe check for the remote real tiny-LLM sharded inference Beta path."""
...
def validate_report(payload: dict, *, mode: str, require_requeue: bool, require_distinct_stage_miners: bool) -> None:
    if payload.get("schema") != "remote_real_llm_sharded_beta_v1" or payload.get("ok") is not True:
        raise SystemExit(f"unexpected remote real LLM sharded Beta report: {json.dumps(payload, sort_keys=True)}")
    ...
    expected_mode_code = (
        ...
        else "local_real_llm_sharded_inference_ready"
    )
    ...
    for fragment in SECRET_FRAGMENTS:
        if fragment in encoded:
            raise SystemExit(f"remote real LLM sharded Beta report leaked sensitive fragment: {fragment}")
    if not (payload.get("artifacts") or {}).get("remote_real_llm_sharded_beta_json", {}).get("present"):
        raise SystemExit("remote real LLM sharded Beta JSON artifact was not reported present")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the remote real tiny-LLM sharded inference Beta check.")
    ...
    parser.add_argument("--stage-mode", choices=["both", "split"], default="split")
...
def main() -> None:
    args = parse_args()
    with tempfile.TemporaryDirectory(prefix="crowdtensor_remote_real_llm_sharded_beta_") as temp:
        command = [
            sys.executable,
        ...
        print(json.dumps({
            "schema": "remote_real_llm_sharded_beta_check_v1",
            "ok": True,
            "mode": args.mode,
