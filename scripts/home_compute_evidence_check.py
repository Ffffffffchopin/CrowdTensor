#!/usr/bin/env python3
"""Acceptance check for the safe home-compute evidence pack."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate the CrowdTensorD home-compute evidence pack.")
...
def main() -> None:
    ...
    command = [
        sys.executable,
        str(ROOT / "scripts" / "home_compute_evidence_pack.py"),
        "--host",
        args.host,
        "--port",
        str(args.port),
    ...
    if completed.returncode != 0:
        raise SystemExit(
            "home_compute_evidence_pack.py failed\n"
            f"stdout:\n{completed.stdout}\n"
            f"stderr:\n{completed.stderr}"
        )
    lines = [line for line in completed.stdout.splitlines() if line.strip()]
    if not lines:
        raise SystemExit("home_compute_evidence_pack.py emitted no JSON evidence")
    ...
    if (
        ...
        raise SystemExit(f"unexpected evidence route: {route}")
    if "target:cpu_baseline" not in (route.get("matched_capabilities") or []):
        raise SystemExit(f"route decision missing CPU target match: {route}")
    if route.get("missing_capabilities"):
        raise SystemExit(f"home-compute evidence route should not miss capabilities: {route}")

    selected = evidence.get("selected_workload") or {}
    ...
    matrix = evidence.get("runtime_matrix") or {}
    targets = {target.get("name"): target for target in matrix.get("hardware_targets", [])}
    if not matrix.get("ok") or not targets.get("cpu_baseline", {}).get("usable_now"):
    ...
    if not safety.get("read_only") or not safety.get("redaction_ok") or safety.get("raw_payloads_exposed"):
        raise SystemExit(f"unsafe evidence report: {safety}")
    ...
    for secret_fragment in ["local-runtime-key", "CROWDTENSOR_LLM_RUNTIME_API_KEY=", "Bearer "]:
        if secret_fragment in payload:
            raise SystemExit("home-compute evidence leaked secret-like material")
