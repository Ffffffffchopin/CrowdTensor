#!/usr/bin/env python3
"""Validate GLM 5.2 DSA-masked single-layer decode reports."""
...
from scripts import glm52_dsa_masked_layer_decode_probe as probe  # noqa: E402


SCHEMA = "glm52_dsa_masked_layer_decode_check_v1"
...
def validate_report(report: dict[str, Any], *, require_verified: bool = False) -> list[str]:
    ...
    if report.get("dsa_indexer_type") != "full":
        errors.append("dsa_indexer_type_not_full")
    for key in [
    ...
    dsa_heads = _int(report.get("dsa_index_n_heads"))
    dsa_head_dim = _int(report.get("dsa_index_head_dim"))
    dsa_topk = _int(report.get("dsa_mask_topk_count"))
    expected_shapes = {
        ...
        "dsa_indexer_query_shape": [updated, dsa_heads, dsa_head_dim],
        "dsa_indexer_key_shape": [updated, dsa_head_dim],
    ...
    if dsa_topk <= 0:
        errors.append("dsa_mask_topk_count_missing")
    if _int(report.get("dsa_mask_pruned_position_count")) <= 0:
        errors.append("dsa_mask_pruned_position_count_missing")
    if dsa_topk + _int(report.get("dsa_mask_pruned_position_count")) != updated:
        errors.append("dsa_mask_topk_pruned_count_mismatch")
    ...
    for key in [
        ...
        "decode_query_hash",
        "raw_attention_scores_hash",
        "attention_scores_hash",
        "attention_weights_hash",
        "attention_output_hash",
        "dsa_index_score_hash",
        "dsa_topk_indices_hash",
        "dsa_attention_mask_hash",
    ...
    if require_verified and report.get("layer_decode_verified") is not True:
        errors.append("dsa_masked_layer_decode_verification_required")
    ...
    for key in [
        "dsa_masked_layer_decode_is_single_layer_only",
        "dsa_masked_layer_decode_uses_small_sequence_topk_cap",
        "dsa_masked_layer_decode_is_not_stage_decode",
        "dsa_masked_layer_decode_is_not_same_request",
        "requires_full_dsa_topk_scale_or_real_sequence",
        "requires_multi_layer_stage_runtime",
        "requires_lm_head_token_selection_from_stage_hidden",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.json:
        ...
        print(
            f"glm52_dsa_masked_layer_decode_check: ok={result['ok']} "
            f"errors={len(errors)} dsa_masked={result['dsa_masked_attention_integrated']}"
        )
        for error in errors:
            print(f"- {error}")
