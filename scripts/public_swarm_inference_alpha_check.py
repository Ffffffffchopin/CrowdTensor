...
def build_check(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not isinstance(prompt_scope.get("prompt_count"), int) or prompt_scope.get("prompt_count") < 0:
        scope_errors.append("prompt_scope_count_mismatch")
    ...
    if prompt_scope.get("terminal_logs_local_private") is not False:
        scope_errors.append("prompt_scope_terminal_logs_local_private_mismatch")
    if prompt_scope.get("saved_artifacts_prompt_placeholders") is not True:
        scope_errors.append("prompt_scope_saved_artifacts_prompt_placeholders_mismatch")
    if prompt_scope.get("saved_artifacts_public_safe") is not True:
        scope_errors.append("prompt_scope_saved_artifacts_public_safe_mismatch")
    if prompt_scope.get("prefer_prompt_file_or_stdin_for_shareable_logs") is not False:
        scope_errors.append("prompt_scope_prefer_prompt_file_or_stdin_mismatch")
    if prompt_scope.get("prompt_file_path_public") is not False:
        scope_errors.append("prompt_scope_prompt_file_path_public_mismatch")
