...
def artifact_summary(output_dir: Path) -> dict[str, Any]:
    paths = {
        "inspect_first": output_dir / "public_swarm_operator_preview.md",
        "summary_json": output_dir / "public_swarm_operator_preview.json",
        "summary_markdown": output_dir / "public_swarm_operator_preview.md",
...
def operator_preview_command(args: argparse.Namespace, output_dir: Path, mode: str) -> list[str]:
    command = [
        "crowdtensor",
        "operator-preview",
        mode,
    ...
    if mode == MODE_LIVE_KAGGLE:
        ...
        command.extend([
            ...
            str(args.release_readiness_report),
            "--gpu-report",
            str(args.gpu_report),
        ])
...
def not_completed_items(report: dict[str, Any]) -> list[str]:
    ...
    live = payloads.get("live_preview") if isinstance(payloads.get("live_preview"), dict) else {}
    release = payloads.get("release_readiness") if isinstance(payloads.get("release_readiness"), dict) else {}
    gpu = payloads.get("gpu_generation_evidence") if isinstance(payloads.get("gpu_generation_evidence"), dict) else {}
    items = [
        ("Operator Preview ready", preview.get("ready")),
        ("operator user path ready", preview.get("user_path_ready")),
        ("Developer Preview ready or degraded CPU fallback", preview.get("developer_preview_ready") or preview.get("developer_preview_degraded") or developer.get("ready")),
        ("Live Preview evidence ready", preview.get("live_preview_ready") or live.get("ready")),
        ("release readiness ready", preview.get("release_readiness_ready") or release.get("ready")),
        ("support bundle ready", preview.get("support_bundle_ready")),
    ...
    if mode == MODE_PACKAGE:
        ...
        items.extend([
            ("external runtime verified or retained evidence imported", preview.get("external_runtime_verified") or preview.get("retained_evidence_ready")),
            ("live attempt not blocked", not preview.get("external_runtime_blocked")),
        ])
    elif mode == MODE_EVIDENCE_IMPORT:
        items.append(("retained live evidence ready", preview.get("retained_evidence_ready")))
    ...
    *,
    ...
    if ready:
        return command_entry(
            "inspect Operator Preview evidence",
            artifact_command(output_dir, "public_swarm_operator_preview.md"),
            reason="review_artifacts",
        )
    if mode == MODE_PACKAGE:
        return command_entry(
            "run local Operator Preview smoke",
            operator_preview_command(args, output_dir, MODE_LOCAL_SMOKE),
            reason="verify_local_operator_path",
        )
    if mode == MODE_LIVE_KAGGLE:
        return command_entry(
            "rerun Operator Preview live Kaggle proof",
            operator_preview_command(args, output_dir, MODE_LIVE_KAGGLE),
            reason="verify_live_kaggle_operator_path",
            requires_private_credentials=True,
            side_effectful=True,
        )
    if mode == MODE_EVIDENCE_IMPORT:
        return command_entry(
            "refresh retained Operator Preview evidence",
            operator_preview_command(args, output_dir, MODE_EVIDENCE_IMPORT),
            reason="refresh_retained_evidence",
        )
    label = "retry local Operator Preview smoke"
    reason = "fix_operator_preview_blockers" if not_completed else "rerun_local_smoke"
    return command_entry(label, operator_preview_command(args, output_dir, MODE_LOCAL_SMOKE), reason=reason)
...
def next_commands(
    ...
    *,
    ...
    if mode == MODE_PACKAGE:
        ...
        commands.append(command_entry(
            "rerun local Operator Preview smoke",
            operator_preview_command(args, output_dir, MODE_LOCAL_SMOKE),
            reason="refresh_local_operator_path",
        ))
    else:
        commands.append(command_entry(
            "retry local Operator Preview smoke",
            operator_preview_command(args, output_dir, MODE_LOCAL_SMOKE),
            reason=str(recommended.get("reason") or "fix_operator_preview_blockers"),
        ))
    return commands
...
def user_status(
    *,
    ...
    if ready:
        ...
        state = "live-blocked"
        headline = "Operator Preview live Kaggle proof needs attention."
        next_step = "fix_live_runtime_or_import_retained_evidence"
    elif mode == MODE_PACKAGE:
        state = "package-blocked"
        headline = "Operator Preview package did not satisfy the package readiness gate."
        next_step = "run_local_or_fix_package"
    else:
        state = "blocked"
        headline = "Public Swarm Operator Preview evidence needs attention."
...
def review_summary(
    ...
    *,
    ...
    attention = "none" if ready else (not_completed[0] if not_completed else "public_swarm_operator_preview_blocked")
    return {
        "schema": "public_swarm_operator_preview_review_summary_v1",
        "state": "ready" if ready else "blocked",
        "headline": "Public Swarm Operator Preview evidence is ready." if ready else "Public Swarm Operator Preview evidence needs attention.",
        "next_step": "review_artifacts" if ready else "fix_blockers",
        "inspect_first": str(output_dir / "public_swarm_operator_preview.md"),
        "support_bundle": str(output_dir / "support_bundle.json"),
        "recommended_label": recommended.get("label") or "none",
        "recommended_reason": recommended.get("reason") or "none",
        "next_command": recommended.get("command_line") or "",
        "primary_code": "public_swarm_operator_preview_ready" if ready else (codes[0] if codes else "public_swarm_operator_preview_blocked"),
        "attention": attention,
