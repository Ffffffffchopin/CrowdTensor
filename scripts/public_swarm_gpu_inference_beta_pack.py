...
def output_request_summary() -> dict[str, Any]:
    return {
        ...
        "summary": (
            "Public Swarm GPU Inference Beta artifacts summarize optional CUDA tiny GPT split "
            "readiness with hashes, counts, lifecycle state, and diagnostics only. "
...
def prompt_uses_inline_text(args: argparse.Namespace) -> bool:
    return getattr(args, "mode", "") in {"local-loopback", "verify"}


def prompt_scope_summary(args: argparse.Namespace) -> dict[str, Any]:
    prompts = parse_prompt_texts(getattr(args, "prompt_texts", "")) if prompt_uses_inline_text(args) else []
    ...
    return {
        ...
        "public_artifact_safe": True,
        "summary": (
            "Public Swarm GPU Inference Beta artifacts record prompt source/count and "
            "placeholder safety only; raw prompt text is excluded from public JSON, "
            "Markdown, and imported evidence summaries."
        ),
    }


def prompt_secret_values(args: argparse.Namespace) -> list[str]:
    if not prompt_uses_inline_text(args):
        return []
...
def answer_scope_summary() -> dict[str, Any]:
    return {
        ...
        "public_artifact_safe": True,
        "summary": (
            "This Public Swarm GPU Inference Beta report is shareable CUDA readiness evidence, "
            "not a local answer transcript; raw prompts, generated text, generated token ids, "
            "activations, leases, credentials, private env files, Kaggle kernel payloads, and runtime state are excluded."
        ),
    }
...
def shareable_summary() -> dict[str, Any]:
    return {
        ...
        "public_artifact_safe": True,
        "summary": (
            "Share public_swarm_gpu_inference_beta_*.json/md artifacts; they contain "
...
def build_report(args: argparse.Namespace, *, runner: Runner = subprocess.run) -> dict[str, Any]:
    ...
    return persist_report(
        ...
        secret_values=[getattr(args, "observer_token", ""), getattr(args, "admin_token", ""), *prompt_secret_values(args)],
    )
