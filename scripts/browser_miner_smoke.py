#!/usr/bin/env python3
"""Playwright smoke test for the CrowdTensorD browser Miner bridge."""
...
def start_web_server(host: str, port: int) -> ReusableThreadingHTTPServer:
    ...
    thread = threading.Thread(target=server.serve_forever, name="browser-miner-web", daemon=True)
    thread.start()
    return server
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the browser Miner bridge smoke test.")
    parser.add_argument("--host", default="127.0.0.1")
    parser.add_argument("--coordinator-port", type=int, default=8898)
...
def main() -> None:
    ...
    if state_dir is None:
        temp_dir = tempfile.TemporaryDirectory(prefix="crowdtensor_browser_miner_")
    ...
    try:
        ...
        with sync_playwright() as playwright:
            ...
            try:
                ...
                raise SystemExit(f"browser Miner smoke timed out\nstatus={json.dumps(status, sort_keys=True)}") from exc

            status = page.evaluate("() => window.__crowdTensorBrowserMinerStatus")
            state = request_json("GET", args.coordinator_url, "/state")
            if state.get("accepted_results", 0) < 1:
                raise SystemExit(f"coordinator did not record accepted result: {state}")
            if not status.get("taskId"):
                raise SystemExit(f"missing browser task id: {status}")
            if status.get("workerElapsedMs", 0) <= 0:
                raise SystemExit(f"invalid worker metrics: {status}")

            print(json.dumps({
                "accepted": bool(status["accepted"]),
                "task_id": status["taskId"],
                "global_step": status["globalStep"],
                "model_version": status["modelVersion"],
                "optimizer_step": status["optimizerStep"],
                "loss": status["loss"],
                "inner_loss_start": status["innerLossStart"],
                "inner_loss_end": status["innerLossEnd"],
