...
class PublicRealLlmSwarmBetaPackTests(unittest.TestCase):
    ...
    def test_release_aggregates_product_external_p2p_and_cuda_fail_closed(self) -> None:
        ...
        self.assertEqual(
            report["runtime_provenance"]["proof_level"],
            "release-local-cpu-with-retained-external-and-local-gpu-smoke",
        )
        self.assertTrue(report["runtime_provenance"]["local_cpu_product_path_ready"])
        self.assertTrue(report["runtime_provenance"]["external_kaggle_cpu_evidence_ready"])
        self.assertTrue(report["runtime_provenance"]["p2p_candidate_evidence_ready"])
        self.assertTrue(report["runtime_provenance"]["local_gpu_smoke_ran"])
        ...
        self.assertEqual(report["inference_verdict"]["kind"], "Public Real-LLM Swarm Beta")
        self.assertEqual(report["inference_verdict"]["state"], "ready")
        ...
        self.assertEqual(report["inference_verdict"]["evidence_level"], "release-local-cpu-with-retained-external")
        self.assertEqual(report["inference_verdict"]["executed_where"], "local-cpu-plus-retained-evidence")
        self.assertEqual(report["inference_verdict"]["gpu_state"], "local-gpu-smoke-only")
        self.assertFalse(report["inference_verdict"]["fresh_kaggle_gpu_verified"])
        self.assertEqual(report["inference_verdict"]["source_inference_verdict"]["kind"], "Public Swarm v2")
        self.assertEqual(report["inference_verdict"]["source_inference_verdict_state"], "ready")
        self.assertTrue(report["inference_verdict"]["source_inference_verdict_completed"])
        self.assertEqual(report["inference_verdict"]["recommended_reason"], "check_current_beta_report")
        ...
        self.assertEqual(report["user_status"]["headline"], "Public Real-LLM Swarm Beta evidence is ready.")
        self.assertEqual(report["user_status"]["next_step"], "run_beta_report_check")
        self.assertEqual(report["user_status"]["recommended_label"], "validate beta report")
        ...
        self.assertEqual(report["review_summary"]["recommended_label"], "validate beta report")
        ...
        self.assertTrue(any(item["label"] == "validate beta report" for item in report["next_commands"]))
        ...
        self.assertIn("- status: `ready` Public Real-LLM Swarm Beta evidence is ready.", markdown)
        self.assertIn("- state: `ready`", markdown)
        self.assertIn("- next step: `run_beta_report_check`", markdown)
        self.assertIn("- inspect first: `public_real_llm_swarm_beta.md`", markdown)
        self.assertIn("- recommended next: `crowdtensor public-real-llm-swarm-beta check", markdown)
        self.assertIn("- recommended check: `crowdtensor public-real-llm-swarm-beta check", markdown)
        self.assertIn("- verdict: `state=ready completed=True preflight_only=False answer=no-local-answer", markdown)
        self.assertIn("gpu=local-gpu-smoke-only fresh_kaggle_gpu=False source=ready", markdown)
        self.assertIn("- verdict note: Public Real-LLM Swarm Beta evidence is ready.", markdown)
        self.assertIn("--beta-report", markdown)
        self.assertIn("## What To Do Next", markdown)
        self.assertIn("- inspect support bundle: `sed -n 1,220p", markdown)
        self.assertIn("- validate beta report: `crowdtensor public-real-llm-swarm-beta check", markdown)
        ...
        self.assertEqual(support["review_summary"]["next_step"], "run_beta_report_check")
        self.assertEqual(support["recommended_check_command"], report["recommended_check_command"])
    ...
    def test_persist_report_prunes_generated_private_runtime_files(self) -> None:
        ...
        self.assertEqual(persisted["inference_verdict"]["schema"], pack.INFERENCE_VERDICT_SCHEMA)
        self.assertEqual(persisted["inference_verdict"]["kind"], "Public Real-LLM Swarm Beta")
        self.assertEqual(persisted["inference_verdict"]["gpu_state"], "no-gpu-evidence")
        self.assertFalse(persisted["inference_verdict"]["fresh_kaggle_gpu_verified"])
    ...
    def test_evidence_import_blocks_when_usable_kv_cache_report_missing(self) -> None:
        ...
        self.assertEqual(report["user_status"]["next_step"], "review_not_completed")
        self.assertEqual(report["recommended_next_command"]["label"], "inspect Not Completed section")
        self.assertTrue(any(item["label"] == "inspect support bundle" for item in report["next_commands"]))
    ...
    def test_check_script_validates_release_contract(self) -> None:
        ...
        self.assertEqual(result["review_summary"]["recommended_next_command"], result["recommended_next_command"])
        self.assertEqual(result["review_summary"]["next_command"], result["recommended_next_command"]["command_line"])
        self.assertEqual(result["user_status"]["state"], "ready")
        self.assertEqual(result["user_status"]["next_step"], "review_checked_artifacts")
        self.assertEqual(result["user_status"]["recommended_label"], result["recommended_next_command"]["label"])
        self.assertEqual(result["recommended_next_command"]["label"], "inspect checked Beta summary")
        self.assertEqual(result["recommended_next_command"]["reason"], "review_checked_artifacts")
        self.assertIn("sed -n 1,220p", result["recommended_next_command"]["command_line"])
        self.assertIn("public_real_llm_swarm_beta.md", result["recommended_next_command"]["command_line"])
        self.assertTrue(any(item["label"] == "inspect support bundle" for item in result["next_commands"]))
        self.assertTrue(any(item["label"] == "inspect check JSON" for item in result["next_commands"]))
        self.assertTrue(any(item["command_line"] == result["recommended_check_command"]["command_line"] for item in result["next_commands"]))
        ...
        self.assertEqual(report["answer_scope"]["scope_state"], "no-local-answer")
        self.assertEqual(report["shareable_summary"]["answer_scope_state"], "no-local-answer")
        self.assertEqual(result["checked_runtime_provenance"], report["runtime_provenance"])
        self.assertEqual(result["checked_evidence_scope"], report["evidence_scope"])
        self.assertEqual(result["checked_evidence_scope"]["level"], "release-local-cpu-with-retained-external")
        self.assertFalse(result["checked_evidence_scope"]["fresh_kaggle_gpu_verified"])
        self.assertEqual(result["checked_inference_verdict"], report["inference_verdict"])
        self.assertEqual(result["checked_inference_verdict"]["state"], "ready")
        self.assertTrue(result["checked_inference_verdict"]["completed"])
        self.assertEqual(result["checked_inference_verdict"]["gpu_state"], "local-gpu-smoke-only")
        self.assertFalse(result["checked_inference_verdict"]["fresh_kaggle_gpu_verified"])
        ...
        self.assertEqual(persisted_check["recommended_next_command"], result["recommended_next_command"])
        self.assertEqual(persisted_check["next_commands"], result["next_commands"])
        self.assertEqual(persisted_check["user_status"], result["user_status"])
        self.assertEqual(persisted_check["checked_evidence_scope"], result["checked_evidence_scope"])
        self.assertEqual(persisted_check["checked_runtime_provenance"], result["checked_runtime_provenance"])
        self.assertEqual(persisted_check["checked_inference_verdict"], result["checked_inference_verdict"])
    ...
    def test_check_validation_requires_user_guidance_fields(self) -> None:
        ...
        payload.pop("user_status", None)
        payload.pop("recommended_next_command", None)
        payload.pop("next_commands", None)
        report_path = output_dir / "beta" / "public_real_llm_swarm_beta.json"
        persisted = json.loads(report_path.read_text(encoding="utf-8"))
        persisted.pop("user_status", None)
        persisted.pop("recommended_next_command", None)
        persisted.pop("next_commands", None)
        report_path.write_text(json.dumps(persisted, indent=2, sort_keys=True) + "\n", encoding="utf-8")
        support_path = output_dir / "beta" / "support_bundle.json"
        support = json.loads(support_path.read_text(encoding="utf-8"))
        support.pop("user_status", None)
        support.pop("recommended_next_command", None)
        support.pop("next_commands", None)
        support_path.write_text(json.dumps(support, indent=2, sort_keys=True) + "\n", encoding="utf-8")

        errors = check.validate_report(payload, mode="release", expected_tokens=16)

        self.assertIn("user_status_state_mismatch", errors)
        self.assertIn("recommended_next_command_missing", errors)
        self.assertIn("next_commands_missing", errors)
    ...
    def test_pack_human_summary_shows_final_status_and_artifacts(self) -> None:
        ...
        self.assertIn("    - Validate this report before sharing: crowdtensor public-real-llm-swarm-beta check", output)
    ...
    def test_check_human_summary_shows_artifacts(self) -> None:
        ...
        self.assertIn("fresh_kaggle_gpu_attempted=False", output)
        self.assertIn("fresh_kaggle_gpu=False", output)
        self.assertIn("  checked_evidence_scope_note:", output)
        self.assertIn("  checked_verdict: state=ready completed=True preflight_only=False answer=no-local-answer", output)
        self.assertIn("gpu=local-gpu-smoke-only fresh_kaggle_gpu=False source=ready", output)
        self.assertIn("  checked_verdict_note: Public Real-LLM Swarm Beta evidence is ready.", output)
        self.assertIn("  recommended_check: crowdtensor public-real-llm-swarm-beta check", output)
        self.assertIn("--beta-report", output)
        self.assertIn(
    ...
    def test_check_script_blocked_result_has_review_guidance(self) -> None:
        ...
        result["recommended_next_command"] = check.recommended_next_command(result)
        result["next_commands"] = check.next_commands(result)
        result["user_status"] = check.user_status(result)
        ...
        self.assertEqual(result["review_summary"]["recommended_next_command"], result["recommended_next_command"])
        self.assertEqual(result["review_summary"]["next_command"], result["recommended_next_command"]["command_line"])
        self.assertEqual(result["user_status"]["state"], "blocked")
        self.assertEqual(result["user_status"]["next_step"], "fix_validation_errors")
        self.assertEqual(result["user_status"]["recommended_label"], "inspect check errors")
        self.assertEqual(result["recommended_next_command"]["label"], "inspect check errors")
        self.assertIn("public_real_llm_swarm_beta_check.json", result["recommended_next_command"]["command_line"])
        self.assertTrue(any(item["label"] == "inspect support bundle" for item in result["next_commands"]))
        ...
        self.assertIn("Open check_json for the validation errors", result["operator_action"])
        self.assertIn("check_json", result["operator_action"])
        self.assertIn("Not Completed", result["operator_action"])
        stdout = io.StringIO()
        with contextlib.redirect_stdout(stdout):
            check.print_human_summary(result)
        output = stdout.getvalue()
        self.assertIn("Public Real-LLM Swarm Beta check ready: False", output)
        self.assertIn("  status: state=blocked next=fix_validation_errors recommended=inspect check errors", output)
