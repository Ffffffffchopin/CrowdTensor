...
class CrowdTensorCliTests(unittest.TestCase):
    ...
    def test_public_swarm_product_beta_help_explains_modes_and_output_scope(self) -> None:
        stdout = io.StringIO()

        with contextlib.redirect_stdout(stdout), self.assertRaises(SystemExit) as raised:
            cli.main(["public-swarm-product-beta", "--help"])

        self.assertEqual(raised.exception.code, 0)
        rendered = stdout.getvalue()
        normalized = " ".join(rendered.split())
        self.assertIn("local-loopback run the localhost Product Beta serve/join/generate proof", normalized)
        self.assertIn("package generate two-machine or Kaggle stage join material and runbook artifacts", normalized)
        self.assertIn("external-existing verify an already running controlled Coordinator plus stage Miners", normalized)
        self.assertIn("The report prints status, review, recommended_next, next[...] commands, output scope", normalized)
        self.assertIn("Public artifacts contain hashes/counts/readiness evidence only", normalized)

    def test_live_preview_help_explains_modes_output_scope_and_side_effects(self) -> None:
        stdout = io.StringIO()

        with contextlib.redirect_stdout(stdout), self.assertRaises(SystemExit) as raised:
            cli.main(["live-preview", "--help"])

        self.assertEqual(raised.exception.code, 0)
        rendered = stdout.getvalue()
        normalized = " ".join(rendered.split())
        self.assertIn("local-smoke run CI-safe Developer Preview and Alpha contract checks", normalized)
        self.assertIn("package generate the runbook/package without creating Kaggle resources", normalized)
        self.assertIn("live-kaggle run the side-effectful public Coordinator plus private Kaggle proof", normalized)
        self.assertIn("evidence-import aggregate retained Developer Preview and Alpha RC evidence", normalized)
    ...
    def test_swarm_trial_help_explains_modes_output_scope_and_side_effects(self) -> None:
        stdout = io.StringIO()

        with contextlib.redirect_stdout(stdout), self.assertRaises(SystemExit) as raised:
            cli.main(["swarm-trial", "--help"])

        self.assertEqual(raised.exception.code, 0)
        rendered = stdout.getvalue()
        normalized = " ".join(rendered.split())
        self.assertIn("local-loopback Run the local Product Beta serve/join/generate path", normalized)
        self.assertIn("package Generate SWARM_TRIAL.md and join/package artifacts", normalized)
        self.assertIn("live-kaggle Run the side-effectful Operator Preview live Kaggle proof", normalized)
        self.assertIn("evidence-import Aggregate retained Product Beta, Operator Preview, and GPU generation evidence reports", normalized)
        self.assertIn("Reports print status, review, recommended_next, next[...] commands, output scope", normalized)
        self.assertIn("live-kaggle is side-effectful and requires cleanup plus token rotation", normalized)
    ...
    def test_print_cpu_infer_beta_rc_outputs_guidance_summary(self) -> None:
        report = {
            "schema": "cpu_inference_beta_rc_v1",
            "ok": True,
            "mode": "beta-rc",
            ...
            "user_status": {
                "state": "ready",
                "headline": "CPU Inference Beta RC evidence is ready.",
                "next_step": "review_artifacts",
                "recommended_label": "inspect CPU Inference Beta RC evidence",
                "recommended_reason": "review_artifacts",
                "public_artifact_safe": True,
            },
            "review_summary": {
                ...
                "state": "ready",
                "next_step": "review_artifacts",
                "inspect_first": "/tmp/cpu-infer-rc/cpu_inference_beta_rc.md",
                "support_bundle": "/tmp/cpu-infer-rc/support_bundle.json",
                "recommended_label": "inspect CPU Inference Beta RC evidence",
                "recommended_reason": "review_artifacts",
                "next_command": "sed -n 1,220p /tmp/cpu-infer-rc/cpu_inference_beta_rc.md",
                "attention": "none",
                "primary_code": "cpu_inference_beta_rc_ready",
                "public_artifact_safe": True,
            },
            "recommended_next_command": {
                "label": "inspect CPU Inference Beta RC evidence",
                "reason": "review_artifacts",
                "command_line": "sed -n 1,220p /tmp/cpu-infer-rc/cpu_inference_beta_rc.md",
            },
            "next_commands": [
                {
                    "label": "inspect shareable summary",
                ...
                {
                    "label": "inspect support bundle",
                    "command_line": "sed -n 1,220p /tmp/cpu-infer-rc/support_bundle.json",
                },
            ],
            "artifact_summary": {
            ...
            "diagnosis_codes": ["cpu_inference_beta_rc_ready"],
            "steps": [{"name": "local_cpu_inference", "ok": True, "payload_schema": "cpu_inference_beta_v1"}],
            "artifacts": {},
        }
        ...
        self.assertIn("CrowdTensor CPU inference Beta RC", rendered)
        self.assertIn("status: ready", rendered)
        self.assertIn("review: state=ready", rendered)
        self.assertIn("recommended_next: inspect CPU Inference Beta RC evidence", rendered)
    ...
    def test_print_swarm_session_outputs_scope_summary(self) -> None:
        report = {
            ...
            "user_status": {
                ...
                "next_step": "review_artifacts",
                "recommended_label": "inspect Public Swarm Alpha evidence",
                "recommended_reason": "review_artifacts",
                "not_completed_count": 0,
                "public_artifact_safe": True,
            },
            "review_summary": {
                ...
                "headline": "Public Swarm Inference Alpha evidence is ready.",
                "next_step": "review_artifacts",
                "inspect_first": "/tmp/public-swarm-alpha/public_swarm_inference_alpha.md",
                "support_bundle": "/tmp/public-swarm-alpha/support_bundle.json",
                "recommended_label": "inspect Public Swarm Alpha evidence",
                "recommended_reason": "review_artifacts",
                "next_command": "sed -n 1,220p /tmp/public-swarm-alpha/public_swarm_inference_alpha.md",
                "attention": "none",
                "not_completed_count": 0,
                "public_artifact_safe": True,
            },
            "recommended_next_command": {
                "label": "inspect Public Swarm Alpha evidence",
                "reason": "review_artifacts",
                "command_line": "sed -n 1,220p /tmp/public-swarm-alpha/public_swarm_inference_alpha.md",
            },
            "next_commands": [
                {
                    "label": "inspect shareable summary",
                ...
                {
                    "label": "inspect support bundle",
                    "command_line": "sed -n 1,220p /tmp/public-swarm-alpha/support_bundle.json",
                },
            ],
            "artifact_summary": {
                "present_artifact_count": 4,
        ...
        self.assertIn("status: ready", rendered)
        self.assertIn("review: state=ready", rendered)
        self.assertIn("recommended_next: inspect Public Swarm Alpha evidence", rendered)
        self.assertIn("next[1] inspect shareable summary", rendered)
        self.assertIn("artifacts: present=4/4", rendered)
        self.assertIn("output_request: include_output=False", rendered)
        self.assertIn("raw_generated_text_public=False", rendered)
    ...
    def test_public_swarm_product_beta_prints_output_scope(self) -> None:
        report = {
            ...
            "user_status": {
                ...
                "next_step": "review_artifacts",
                "recommended_label": "inspect Product Beta evidence",
                "recommended_reason": "review_artifacts",
                "not_completed_count": 0,
                "public_artifact_safe": True,
            },
            "review_summary": {
                ...
                "state": "ready",
                "next_step": "review_artifacts",
                "inspect_first": "dist/product-beta/public_swarm_product_beta.md",
                "support_bundle": "dist/product-beta/support_bundle.json",
                "recommended_label": "inspect Product Beta evidence",
                "recommended_reason": "review_artifacts",
                "next_command": "sed -n 1,220p dist/product-beta/public_swarm_product_beta.md",
                "primary_code": "public_swarm_product_beta_ready",
                "attention": "none",
                "public_artifact_safe": True,
            },
            "recommended_next_command": {
                "label": "inspect Product Beta evidence",
                "reason": "review_artifacts",
            ...
            "next_commands": [
                {
                    "label": "inspect support bundle",
                    "command_line": "sed -n 1,220p dist/product-beta/support_bundle.json",
                }
            ],
            "artifact_summary": {
                "artifact_count": 4,
        ...
        self.assertIn("  status: ready: Public Swarm Product Beta evidence is ready.", output)
        self.assertIn("  review: state=ready next=review_artifacts", output)
        self.assertIn(
            "  review_next: label=inspect Product Beta evidence reason=review_artifacts command=sed -n 1,220p dist/product-beta/public_swarm_product_beta.md",
            output,
        )
        self.assertIn(
        ...
        self.assertIn("  next[1] inspect support bundle: sed -n 1,220p dist/product-beta/support_bundle.json", output)
        self.assertIn("  artifacts: present=4/4 support=dist/product-beta/support_bundle.json public_artifact_safe=True", output)
    ...
    def test_print_live_preview_outputs_scope_summary(self) -> None:
        report = {
            ...
            "user_status": {
                "state": "ready",
                "headline": "Public Swarm Live Preview RC evidence is ready.",
                "next_step": "review_artifacts",
                "recommended_label": "inspect Live Preview RC evidence",
                "recommended_reason": "review_artifacts",
                "not_completed_count": 0,
                "public_artifact_safe": True,
            },
            "review_summary": {
                "schema": "public_swarm_live_preview_rc_review_summary_v1",
                "state": "ready",
                "next_step": "review_artifacts",
                "inspect_first": "dist/live-preview/public_swarm_live_preview_rc.md",
                "support_bundle": "dist/live-preview/support_bundle.json",
                "recommended_label": "inspect Live Preview RC evidence",
                "recommended_reason": "review_artifacts",
                "next_command": "sed -n 1,220p dist/live-preview/public_swarm_live_preview_rc.md",
                "primary_code": "public_swarm_live_preview_rc_ready",
                "attention": "none",
                "public_artifact_safe": True,
            },
            "recommended_next_command": {
                "label": "inspect Live Preview RC evidence",
                "reason": "review_artifacts",
                "command_line": "sed -n 1,220p dist/live-preview/public_swarm_live_preview_rc.md",
            },
            "next_commands": [
                {
                    "label": "inspect support bundle",
                    "command_line": "sed -n 1,220p dist/live-preview/support_bundle.json",
                }
            ],
            "artifact_summary": {
                "artifact_count": 4,
        ...
        rendered = "\n".join(str(call.args[0]) for call in mocked_print.call_args_list)
        self.assertIn("status: ready: Public Swarm Live Preview RC evidence is ready.", rendered)
        self.assertIn("review: state=ready next=review_artifacts", rendered)
        self.assertIn(
            "review_next: label=inspect Live Preview RC evidence reason=review_artifacts command=sed -n 1,220p dist/live-preview/public_swarm_live_preview_rc.md",
            rendered,
        )
        self.assertIn(
            "recommended_next: inspect Live Preview RC evidence reason=review_artifacts sed -n 1,220p dist/live-preview/public_swarm_live_preview_rc.md",
            rendered,
        )
        self.assertIn("output_request: include_output=False", rendered)
        self.assertIn("answer_scope: state=no-local-answer", rendered)
        self.assertIn("shareable: saved_artifacts=True", rendered)
        self.assertIn("next[1] inspect support bundle: sed -n 1,220p dist/live-preview/support_bundle.json", rendered)
        self.assertIn("artifacts: present=4/4 support=dist/live-preview/support_bundle.json public_artifact_safe=True", rendered)
    ...
    def test_public_swarm_trial_prints_output_scope(self) -> None:
        report = {
            ...
            "user_status": {
                "state": "ready",
                "headline": "Public Swarm Trial evidence is ready.",
                "next_step": "review_artifacts",
                "recommended_label": "inspect Swarm Trial evidence",
                "recommended_reason": "review_artifacts",
                "not_completed_count": 0,
                "public_artifact_safe": True,
            },
            "review_summary": {
                "schema": "public_swarm_trial_review_summary_v1",
                "state": "ready",
                "next_step": "review_artifacts",
                "inspect_first": "dist/swarm-trial/public_swarm_trial.md",
                "support_bundle": "dist/swarm-trial/support_bundle.json",
                "recommended_label": "inspect Swarm Trial evidence",
                "recommended_reason": "review_artifacts",
                "next_command": "sed -n 1,220p dist/swarm-trial/public_swarm_trial.md",
                "primary_code": "public_swarm_trial_ready",
                "attention": "none",
                "public_artifact_safe": True,
            },
            "recommended_next_command": {
                "label": "inspect Swarm Trial evidence",
                "reason": "review_artifacts",
                "command_line": "sed -n 1,220p dist/swarm-trial/public_swarm_trial.md",
            },
            "next_commands": [
                {
                    "label": "inspect support bundle",
                    "command_line": "sed -n 1,220p dist/swarm-trial/support_bundle.json",
                }
            ],
            "artifact_summary": {
                "artifact_count": 4,
        ...
        self.assertIn("  status: ready: Public Swarm Trial evidence is ready.", output)
        self.assertIn("  review: state=ready next=review_artifacts", output)
        self.assertIn(
            "  review_next: label=inspect Swarm Trial evidence reason=review_artifacts command=sed -n 1,220p dist/swarm-trial/public_swarm_trial.md",
            output,
        )
        self.assertIn(
            "  recommended_next: inspect Swarm Trial evidence reason=review_artifacts sed -n 1,220p dist/swarm-trial/public_swarm_trial.md",
            output,
        )
        self.assertIn(
            "  output_request: include_output=False raw_generated_text_public=False public_artifact_safe=True",
        ...
        self.assertIn("  next[1] inspect support bundle: sed -n 1,220p dist/swarm-trial/support_bundle.json", output)
        self.assertIn("  artifacts: present=4/4 support=dist/swarm-trial/support_bundle.json public_artifact_safe=True", output)
    ...
    def test_print_micro_llm_live_rc_outputs_guidance_summary(self) -> None:
        report = {
            "schema": "micro_llm_live_rc_v1",
            "cli_schema": "micro_llm_live_rc_cli_v1",
            "ok": True,
            "mode": "local-generated",
            "coordinator_url": "http://127.0.0.1:9180",
            "output_dir": str(Path(self._tmp_dir())),
            "user_status": {
                "state": "ready",
                "headline": "Micro-LLM Live RC evidence is ready.",
                "next_step": "review_artifacts",
                "recommended_label": "inspect micro-LLM Live RC evidence",
                "recommended_reason": "review_artifacts",
                "public_artifact_safe": True,
            },
            "review_summary": {
                ...
                "state": "ready",
                "next_step": "review_artifacts",
                "inspect_first": "/tmp/micro-llm-live-rc/micro_llm_live_rc.md",
                "support_bundle": "/tmp/micro-llm-live-rc/support_bundle.json",
                "recommended_label": "inspect micro-LLM Live RC evidence",
                "recommended_reason": "review_artifacts",
                "next_command": "sed -n 1,220p /tmp/micro-llm-live-rc/micro_llm_live_rc.md",
                "attention": "none",
                "primary_code": "micro_llm_live_rc_ready",
                "public_artifact_safe": True,
            },
            "recommended_next_command": {
                "label": "inspect micro-LLM Live RC evidence",
                "reason": "review_artifacts",
                "command_line": "sed -n 1,220p /tmp/micro-llm-live-rc/micro_llm_live_rc.md",
            },
            "next_commands": [
                {
                    "label": "inspect shareable summary",
                ...
                {
                    "label": "inspect support bundle",
                    "command_line": "sed -n 1,220p /tmp/micro-llm-live-rc/support_bundle.json",
                },
            ],
            "artifact_summary": {
                "present_artifact_count": 4,
            ...
            "diagnosis_codes": ["micro_llm_live_rc_ready"],
        ...
        with patch("builtins.print") as mocked_print:
            cli.print_micro_llm_live_rc(report)

        rendered = "\n".join(str(call.args[0]) for call in mocked_print.call_args_list)
        self.assertIn("status: ready", rendered)
        self.assertIn("review: state=ready", rendered)
        self.assertIn("recommended_next: inspect micro-LLM Live RC evidence", rendered)
        self.assertIn("next[1] inspect shareable summary", rendered)
        self.assertIn("artifacts: present=4/4", rendered)
        self.assertIn("prompt_scope: source=built-in-fixed-scenario", rendered)
        self.assertIn("output_request: include_output=False", rendered)
        self.assertIn("raw_generated_text_public=False", rendered)
        self.assertIn("answer_scope: state=no-local-answer", rendered)
        self.assertIn("shareable: saved_artifacts=True", rendered)
