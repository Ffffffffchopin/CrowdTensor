...
class CrowdTensorCliTests(unittest.TestCase):
    ...
    def test_infer_help_shows_user_examples_and_boundaries(self) -> None:
        ...
        self.assertIn("answer_scope is terminal-only", rendered)
        self.assertIn("was not written to saved JSON/Markdown", rendered)
    ...
    def test_user_docs_manual_demo_sets_tokens_before_submit(self) -> None:
        ...
        for rendered in [readme, quickstart]:
            ...
            self.assertIn("visible answer is terminal-only", rendered)
            self.assertIn("JSON and", rendered)
            self.assertIn("summaries keep hashes/placeholders", rendered)
