...
class CrowdTensorCliTests(unittest.TestCase):
    ...
    def test_product_serve_redacts_tokens_in_command_report(self) -> None:
        ...
        self.assertIn(
            "crowdtensor generate --max-new-tokens 16 --coordinator-url http://127.0.0.1:8787 --dry-run --observer-token '<redacted>'",
            next_lines,
        )
        ...
        self.assertIn("  next[4] check generation route: crowdtensor generate --max-new-tokens 16 --coordinator-url http://127.0.0.1:8787 --dry-run --observer-token '<redacted>'", rendered)
    ...
    def test_product_generate_dry_run_uses_session_protocol(self) -> None:
        ...
        next_lines = [item["command_line"] for item in report["next_commands"]]
        self.assertIn(
            "crowdtensor generate --max-new-tokens 16 --coordinator-url http://127.0.0.1:8787 --backend cuda --hf-model-id distilgpt2 --prompt-text '<prompt>' --dry-run --observer-token '<redacted>'",
            next_lines,
        )
        self.assertIn(
    ...
    def test_generate_accepts_positional_prompt_like_infer(self) -> None:
        ...
        next_lines = [item["command_line"] for item in report["next_commands"]]
        self.assertIn(
            "crowdtensor generate --max-new-tokens 2 --coordinator-url http://127.0.0.1:8787 --prompt-text '<prompt>' --dry-run --observer-token '<redacted>'",
            next_lines,
        )
    ...
    def test_product_generate_dry_run_checks_coordinator_and_stage_preflight(self) -> None:
        ...
        self.assertIn("  next[1] check generation route: crowdtensor generate --max-new-tokens 16 --coordinator-url http://127.0.0.1:8787 --prompt-text '<prompt>' --dry-run --observer-token '<redacted>'", rendered)
        self.assertIn("# requires CROWDTENSOR_OBSERVER_TOKEN", rendered)
    ...
    def test_product_serve_p2p_announces_coordinator(self) -> None:
        ...
        next_lines = [item["command_line"] for item in report["next_commands"]]
        self.assertIn(
            "crowdtensor generate --max-new-tokens 16 --p2p --peer-bootstrap http://127.0.0.1:8788 --dry-run --observer-token '<redacted>'",
            next_lines,
        )
    ...
    def test_product_join_human_output_includes_action_and_redacts_token(self) -> None:
        ...
        self.assertIn("  action: Rerun with --run", rendered)
        self.assertIn("  next[3] check generation route: crowdtensor generate --max-new-tokens 16 --coordinator-url http://127.0.0.1:8787 --dry-run --observer-token '<redacted>'", rendered)
        self.assertIn("# requires CROWDTENSOR_OBSERVER_TOKEN", rendered)
        self.assertIn("# requires CROWDTENSOR_MINER_TOKEN", rendered)
        self.assertNotIn("miner-secret", rendered)
    ...
    def test_product_generate_p2p_discovery_unreachable_returns_actionable_report(self) -> None:
        ...
        self.assertIn(
            "crowdtensor generate --max-new-tokens 2 --p2p --peer-bootstrap http://127.0.0.1:8799 --prompt-text '<prompt>' --dry-run --observer-token '<redacted>'",
            next_lines,
        )
        stdout = io.StringIO()
        with contextlib.redirect_stdout(stdout):
    ...
    def test_product_generate_p2p_preserves_swarm_id_in_next_commands(self) -> None:
        ...
        self.assertIn(
            "crowdtensor generate --max-new-tokens 2 --p2p --swarm-id public-swarm-v2 --peer-bootstrap http://127.0.0.1:8799 --prompt-text '<prompt>' --dry-run --observer-token '<redacted>'",
            next_lines,
        )
    ...
    def test_product_generate_p2p_non_dry_run_blocks_when_route_unusable(self) -> None:
        ...
        next_lines = [item["command_line"] for item in report["next_commands"]]
        self.assertIn(
            "crowdtensor generate --max-new-tokens 2 --p2p --peer-bootstrap http://127.0.0.1:8788 --prompt-text '<prompt>' --dry-run --observer-token '<redacted>'",
            next_lines,
        )
        self.assertIn(
