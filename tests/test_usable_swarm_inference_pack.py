...
class UsableSwarmInferencePackTests(unittest.TestCase):
    ...
    def test_local_mode_requires_eight_token_p2p_real_generate_and_rescue(self) -> None:
        ...
        self.assertTrue(any("USABLE_SWARM_INFERENCE.md" in line for line in next_lines))
        self.assertTrue(any("crowdtensor usable-swarm local" in line for line in next_lines))
        ...
        self.assertEqual(support["user_status"]["state"], "completed")
        self.assertEqual(support["review_summary"]["state"], "completed")
        self.assertEqual(support["recommended_next_command"]["label"], "inspect shareable summary")
    ...
    def test_check_script_validates_package_contract(self) -> None:
        ...
        self.assertEqual(report["review_summary"]["next_step"], "run_local_proof")
        self.assertEqual(report["recommended_next_command"]["label"], "run local usable swarm proof")
        self.assertTrue(any(item["label"] == "run local usable swarm proof" for item in report["next_commands"]))
        self.assertEqual(report["prompt_scope"]["source"], "prompt-text")
        self.assertEqual(report["prompt_scope"]["prompt_count"], 1)
        self.assertEqual(report["answer_scope"]["scope_state"], "no-local-answer")
        self.assertEqual(report["shareable_summary"]["answer_scope_state"], "no-local-answer")
