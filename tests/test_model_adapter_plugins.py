...
from crowdtensor.cli import main as run_cli
...
def test_public_cli_lists_and_checks_discovered_plugin(
    monkeypatch, capsys
) -> None:
    install_fake_points(
        monkeypatch,
    ...
    with pytest.raises(SystemExit) as listed_exit:
        run_cli(["adapters", "list", "--json"])
    assert listed_exit.value.code == 0
    listed = json.loads(capsys.readouterr().out)
    assert listed["ok"] is True
    assert listed["plugin_adapter_count"] == 1
    assert "example" in listed["supported_model_families"]
    with pytest.raises(SystemExit) as checked_exit:
        run_cli(["adapters", "check", "example_lora_v1", "--json"])
    assert checked_exit.value.code == 0
