...
class PublicSwarmInferenceBetaRcPackTests(unittest.TestCase):
    ...
    def test_check_builds_ready_local_loopback_rc_with_fake_runner(self) -> None:
        ...
        self.assertEqual(report["recommended_next_command"]["label"], "inspect Beta RC evidence")
        self.assertIn("public_swarm_inference_beta_rc.md", report["recommended_next_command"]["command_line"])
        ...
        self.assertEqual(support["schema"], "public_swarm_inference_beta_rc_support_bundle_v1")
        self.assertEqual(support["review_summary"]["state"], "ready")
        self.assertEqual(support["recommended_next_command"]["label"], "inspect Beta RC evidence")
        ...
        self.assertIn("- recommended: `inspect Beta RC evidence` reason=`review_artifacts`", markdown)
    ...
    def test_common_report_blocks_when_generate_loop_missing(self) -> None:
        ...
        self.assertIn("mode proof ready", report["not_completed"])
        self.assertIn("serve/join/generate loop ready", report["not_completed"])
        self.assertEqual(report["recommended_next_command"]["label"], "rerun local Beta RC proof")
