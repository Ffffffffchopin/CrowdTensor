...
class MinerInviteTests(unittest.TestCase):
    ...
    def test_duplicate_requires_replace(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(replaced["env"]["CROWDTENSOR_MINER_TOKEN"], "second")
            self.assertNotIn("first", registry.read_text(encoding="utf-8"))
            self.assertNotIn("second", registry.read_text(encoding="utf-8"))
