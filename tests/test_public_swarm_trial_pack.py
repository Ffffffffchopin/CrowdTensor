...
class PublicSwarmTrialPackTests(unittest.TestCase):
    ...
    def test_local_loopback_reports_real_serve_join_generate_ready(self) -> None:
        ...
        self.assertEqual(saved["user_status"]["state"], "ready")
        self.assertEqual(saved["user_status"]["recommended_label"], "inspect Swarm Trial evidence")
        self.assertEqual(saved["review_summary"]["schema"], "public_swarm_trial_review_summary_v1")
        self.assertEqual(saved["review_summary"]["next_step"], "review_artifacts")
        self.assertEqual(saved["recommended_next_command"]["label"], "inspect Swarm Trial evidence")
        self.assertTrue(any(item["label"] == "inspect support bundle" for item in saved["next_commands"]))
        self.assertEqual(saved["artifact_summary"]["present_artifact_count"], saved["artifact_summary"]["artifact_count"])
        self.assertTrue(saved["artifact_summary"]["public_artifact_safe"])
        ...
        self.assertIn("- recommended: `inspect Swarm Trial evidence` reason=`review_artifacts`", markdown)
        ...
        self.assertEqual(support["user_status"], saved["user_status"])
        self.assertEqual(support["review_summary"], saved["review_summary"])
        self.assertEqual(support["artifact_summary"], saved["artifact_summary"])
        self.assertEqual(support["recommended_next_command"]["label"], "inspect Swarm Trial evidence")
    ...
    def test_evidence_import_uses_retained_reports_without_sensitive_fragments(self) -> None:
        ...
        self.assertEqual(report["recommended_next_command"]["label"], "inspect Swarm Trial evidence")
