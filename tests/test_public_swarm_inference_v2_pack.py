...
class PublicSwarmInferenceV2PackTests(unittest.TestCase):
    ...
    def test_fresh_external_16_token_report_satisfies_v2_gate(self) -> None:
        ...
        self.assertTrue(report["runtime_provenance"]["retained_external_evidence_imported"])
        self.assertTrue(report["runtime_provenance"]["retained_external_evidence_ready"])
        self.assertTrue(report["runtime_provenance"]["fresh_external_attempted"])
        self.assertTrue(report["runtime_provenance"]["fresh_external_verified"])
        self.assertTrue(report["runtime_provenance"]["retained_gpu_evidence_imported"])
        self.assertTrue(report["runtime_provenance"]["cuda_optional_fail_closed_ready"])
        self.assertEqual(report["inference_verdict"]["schema"], pack.INFERENCE_VERDICT_SCHEMA)
        self.assertEqual(report["inference_verdict"]["kind"], "Public Swarm v2")
        ...
        self.assertEqual(report["inference_verdict"]["evidence_level"], "retained-evidence-import")
        self.assertEqual(report["inference_verdict"]["gpu_state"], "retained-gpu-evidence")
        self.assertTrue(report["inference_verdict"]["retained_gpu_evidence_imported"])
        self.assertFalse(report["inference_verdict"]["fresh_kaggle_gpu_verified"])
        self.assertTrue(report["inference_verdict"]["fresh_external_verified"])
        self.assertEqual(report["inference_verdict"]["recommended_reason"], "v2_ready")
        ...
        self.assertIn("- verdict: `state=ready completed=True preflight_only=False answer=no-local-answer answer_visible=False artifacts_public=True evidence=retained-evidence-import gpu=retained-gpu-evidence fresh_kaggle_gpu=False next=review_artifacts recommended=review v2 evidence public_artifact_safe=True`", markdown)
        self.assertIn("- verdict note: Public Swarm v2 inference evidence is ready.", markdown)
        ...
        self.assertIn("- proof: `retained-evidence-import`", markdown)
        self.assertIn("- fresh external verified: `True`", markdown)
    ...
    def test_fresh_external_blocks_when_stage_rows_below_token_target(self) -> None:
        ...
        self.assertEqual(report["inference_verdict"]["state"], "blocked")
        self.assertFalse(report["inference_verdict"]["completed"])
        self.assertEqual(report["inference_verdict"]["recommended_reason"], "refresh_external_p2p_evidence")
        self.assertFalse(report["inference_verdict"]["fresh_kaggle_gpu_verified"])
