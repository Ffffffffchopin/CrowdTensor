...
class PublicSwarmGpuInferenceBetaPackTests(unittest.TestCase):
    ...
    def _assert_output_scope(self, report: dict, output_dir: Path, mode: str) -> None:
        ...
        self.assertIn("user_status", report)
        self.assertIn("recommended_next_command", report)
        self.assertIn("next_commands", report)
        self.assertIn("artifact_summary", report)
        self.assertEqual(report["review_summary"]["schema"], "public_swarm_gpu_inference_beta_review_summary_v1")
        ...
        self.assertTrue(report["artifact_summary"]["inspect_first"].endswith(f"public_swarm_gpu_inference_beta_{mode.replace('-', '_')}.md"))
        self.assertTrue(report["artifact_summary"]["summary_json"].endswith(f"public_swarm_gpu_inference_beta_{mode.replace('-', '_')}.json"))
