...
class PublicSwarmProductRcPackTests(unittest.TestCase):
    def test_check_builds_ready_rc_from_synthetic_evidence(self) -> None:
        ...
        self.assertEqual(report["recommended_next_command"]["label"], "inspect Product RC evidence")
        self.assertIn("public_swarm_product_rc.md", report["recommended_next_command"]["command_line"])
        ...
        self.assertEqual(support["schema"], "public_swarm_product_rc_support_bundle_v1")
        self.assertEqual(support["review_summary"]["state"], "ready")
        self.assertEqual(support["recommended_next_command"]["label"], "inspect Product RC evidence")
        ...
        self.assertIn("- recommended: `inspect Product RC evidence` reason=`review_artifacts`", markdown)
        self.assertIn("- not completed count: `0`", markdown)
    ...
    def test_pack_blocks_missing_gpu_evidence(self) -> None:
        ...
        self.assertIn("GPU generation evidence import ready", report["not_completed"])
        self.assertEqual(report["recommended_next_command"]["reason"], "provide_gpu_generation_evidence")
        self.assertIn("PROMPT_TEXT", report["recommended_next_command"]["command_line"])
        self.assertNotIn("private missing gpu prompt", report["recommended_next_command"]["command_line"])
        self.assertEqual(report["prompt_scope"]["source"], "prompt-text")
