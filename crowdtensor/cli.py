...
def gpu_status_summary(provenance: dict[str, Any], scope: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    local_cpu = bool(
        scope.get("local_cpu")
        or (
            scope.get("local_loopback_coordinator")
            and not scope.get("dry_run")
...
def build_sharded_inference(args: argparse.Namespace, *, runner: Runner = subprocess.run) -> dict[str, Any]:
    ...
    summary_json.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    summary["artifacts"]["sharded_inference_evidence_json"]["present"] = evidence_json.is_file()
    summary["artifacts"]["sharded_inference_evidence_markdown"]["present"] = evidence_md.is_file()
...
def build_real_llm_sharded_inference(args: argparse.Namespace, *, runner: Runner = subprocess.run) -> dict[str, Any]:
    ...
    summary_json.write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    summary["artifacts"]["real_llm_sharded_evidence_json"]["present"] = evidence_json.is_file()
...
def print_real_p2p_swarm_inference_core_rc(report: dict[str, Any]) -> None:
    ...
    if recommended:
        suffix = " side_effectful=True" if recommended.get("side_effectful") else ""
        if recommended.get("requires_private_credentials"):
            suffix += " requires_private_credentials=True"
        print(
            "  recommended_next: "
            f"{recommended.get('label')} reason={recommended.get('reason')} {recommended.get('command_line')}{suffix}"
        )
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    infer = subparsers.add_parser(
        ...
        description=(
            ...
            "follow-up commands.\n"
            "The evidence_scope line is the shortest answer to what actually ran: local-cpu-loopback\n"
            "means the fast local CPU proof executed; local-full-evidence means the local full\n"
            "evidence gate executed; existing-runtime-preflight means request shape/readiness\n"
            "was checked without submitting work; existing-runtime-submit means an existing\n"
            "Coordinator/generate route was used; retained_gpu=True means imported/retained\n"
            "GPU evidence was referenced; fresh_kaggle_gpu_attempted=True only means the\n"
            "source report attempted that path; fresh_kaggle_gpu=True is the only claim\n"
            "that a fresh Kaggle GPU proof was verified by the source report. The adjacent\n"
            "evidence_scope_note line is the human-readable explanation of that scope.\n"
    ...
    generate = subparsers.add_parser(
        ...
        description=(
            ...
            "GPU evidence was imported by this front door; fresh_kaggle_gpu_attempted=True\n"
            "only means the source report attempted that path;\n"
            "fresh_kaggle_gpu=True is the only claim that a fresh Kaggle GPU proof was verified. The adjacent\n"
            "evidence_scope_note line is the human-readable explanation of that scope.\n"
    ...
    public_real_llm_swarm_beta = subparsers.add_parser(
        ...
        description=(
            ...
            "the verified report as checked_evidence_scope and prints checked_runtime_provenance\n"
            "for the detailed source/proof summary. Read checked_evidence_scope for the shortest\n"
